...
#ifndef PBRT_FILM_SPECTRAL_IMAGE_NO_CAMERA_H
#define PBRT_FILM_SPECTRAL_IMAGE_NO_CAMERA_H
...
// SpectralImageNoCameraFilm Declarations
class SpectralImageNoCameraFilm : public Film {
public:
    // SpectralImageNoCameraFilm Public Methods
    SpectralImageNoCameraFilm(int xres, int yres, Filter *filt, const float crop[4],
              const string &filename, bool openWindow);
    ~SpectralImageNoCameraFilm() {
        delete pixels;
        delete filter;
        delete[] filterTable;
    }
...
private:
    ...
    // SpectralImageNoCameraFilm Private Data
    Filter *filter;
    float cropWindow[4];
    string filename;
...
#endif // PBRT_FILM_SPECTRAL_IMAGE_NO_CAMERA_H
